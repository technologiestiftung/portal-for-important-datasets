let fs = require('fs'),
  ...
  listAllObjects = require('s3-list-all-objects'),
  turf = require('turf'),
...
app.get('/'+config.secret+'/update', function(req, res) {
  ...
  let datasets = []
    ...
    datasets.forEach((d,di)=>{
      ...
      data.forEach((dd) => {
        if(dd.Key.indexOf(d.file) === 0){
          if(dd.Key.length > 2 && ignores.indexOf(dd.Key.split("/")[1]) == -1 && dd.Key != d.file+"/" && dd.Key != d.file){
            let name = dd.Key.split('.'),
      ...
      data.forEach((dd) => {
        if(dd.Key.indexOf(d.file) === 0 && dd.Key.indexOf("thumb.jpg") > -1){
      ...
      data.forEach((dd) => {
        if(dd.Key.indexOf(d.file) === 0 && dd.Key.indexOf("preview.jpg") > -1){
      ...
      files.forEach(f=>{
        ...
        downloadlist += '<li><a href="' + dataFolder + d.file + "/" + f.file + '"><span class="download-icon"></span><span class="download-title">'+ transType +'</span></a></li>' // <br /><span class="download-subtitle">' + fileSize(dataFolder + "/" + d.file + "/" + f.file) + '&nbsp;MB</span>
      })
      ...
      if(filters.provider.indexOf(d.meta.provider)==-1){
        ...
        url = {de:dataFolder + d.file, en: dataFolder+d.file+"/en"}
      ...
      langs.forEach((l,li)=>{
        ...
        if(isGeoJson){
          let geojson = JSON.parse(request('GET', dataFolder+isGeoJson).getBody());
        ...
        files.forEach((f)=>{
          t_meta[l]["dcat:Dataset"][0]["dcat:distribution"].push(dataFolder+d.file+"/"+f.file)
        })
        ...
        if (!fs.existsSync(__dirname + '/data/' + d.file)){
          fs.mkdirSync(__dirname + '/data/' + d.file);
        }

        fs.writeFileSync( __dirname + '/data/' + d.file + '/dcat'+((l=='en')?'_en':'')+'.json', JSON.stringify(t_meta[l]), 'utf8')

        if(datasets[di].meta['hasPreview']){
          t_data_html[l] = t_data_html[l].split('{{PREVIEW}}').join('<img src="' + dataFolder + d.file + '/preview.jpg" alt="Preview" id="preview" />');
        }else{
        ...
        //   t_data_html[l] = t_data_html[l].split('{{MAP}}').join('<div id="map" data-geojson="' + dataFolder + datasets[di].meta['isGeoJson'] + '"></div>');
        // }else{
        ...
        if(datasets[di].meta['hasThumb']){
          datalist[l] += '<img src="'+dataFolder+d.file+'/thumb.jpg" alt="'+d.meta[l].title+'" />'+"\n";
        }else{
...
app.get('/dataset/:dataset/:lang', function(req, res) {
	...
	if (fs.existsSync(__dirname + '/http/data/'+dataset+lang+'.html')) {
...
app.get('/dataset/:dataset', function(req, res) {
	...
	if (fs.existsSync(__dirname + '/http/data/'+dataset+'.html')) {
